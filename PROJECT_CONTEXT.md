...
- Physical-device QA found a blocking rejected-entry navigation defect: rejecting the active candidate left its match readable, and crossing into the rejected entry persisted the blocked position before showing a stable decision.
- The branch now invalidates only the exact active resolution when that candidate is rejected and persists automatic boundary progress only after the target is safely readable. Focused local tests and the complete local validation set pass; a rebuilt-device retest remains required before other PR #15 QA resumes.
- Remaining PR #15 device QA begins with reject → boundary → Review/Skip/Stop and Back/Resume retesting, followed by completed-list restart, final-list completion, unavailable-source handling, and final narrow-screen or accessibility observations.
